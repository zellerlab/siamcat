...
model.interpretation.plot <- function(siamcat, fn.plot, color.scheme = "BrBG",
    ...
    color.scheme <- rev(colorRampPalette(brewer.pal(
        brewer.pal.info[color.scheme, "maxcolors"], color.scheme))(100))
    ...
    sel.idx <- model.interpretation.select.features(weights = all.weights, model.type = model.type, 
        consens.thres = consens.thres,
        label = label, norm.models = norm.models, max.show = max.show, 
        verbose = verbose)
    ...
    if (heatmap.type == "zscore") {
        ...
        if (is.null(detect.lim)) {
            warning("WARNING: Pseudo-count before log-transformation
             not supplied! Estimating it as 5% percentile...")
            detect.lim <- quantile(feat[feat != 0], 0.05)
        }
        img.data <- model.interpretation.prepare.heatmap.fc(heatmap.data = feat[, srt.idx], 
            sel.feat = names(sel.idx), limits = limits,
            meta = meta(siamcat), label = label, detect.lim = detect.lim, 
            erbose = verbose)
    } else {
    ...
    plot(NULL, type = "n", xlim = c(0, length(hm.label)), xaxs = "i", 
        xaxt = "n", ylim = c(-0.5, 0.5), yaxs = "i", yaxt = "n", xlab = "", 
        ylab = "", bty = "n")
    ul <- unique(hm.label)
    ...
    mtext("Metagenomic Features", side = 3, line = 2, at = length(hm.label)/2,
        cex = 1, adj = 0.5)
    ...
    barplot(as.matrix(rep(1, 100)), col = color.scheme, horiz = TRUE, 
    ...
    model.interpretation.feature.weights.plot(rel.weights = rel.weights, 
                                                sel.idx = sel.idx, 
                                                mod.type = model.type,
                                                label = label)
    ...
    if (model.type != "RandomForest") {
        model.interpretation.heatmap.plot(image.data = img.data, 
            limits = limits, 
        ...
        model.interpretation.heatmap.plot(image.data = img.data, 
            limits = limits, 
            color.scheme = color.scheme, effect.size = NULL, verbose = verbose)
    }
    ...
    model.interpretation.pred.and.meta.plot(prediction = mean.agg.pred[srt.idx], 
        label = label, 
        meta = meta(siamcat)[srt.idx,], verbose = verbose)

    tmp <- dev.off()
    e.time <- proc.time()[3]
    if (verbose > 1)
        message(paste("+ finished model.interpretation.plot in", 
...
#' @keywords internal
model.interpretation.feature.weights.plot <- function(rel.weights, sel.idx, 
    mod.type, label, verbose = 0) {
    if (verbose > 2)
    ...
    if (mod.type != "RandomForest") {
        ...
        barplot(-med, horiz = TRUE, width = 1, space = 0, yaxs = "i", 
            col = "gray30", xlim = c(-max(abs(mi), abs(mx)),
            max(abs(mi), max(mx))), 
            ylim = c(0, dim(relative.weights)[1]), xlab = "", ylab = "",
             yaxt = "n")
        x <- par("usr")
        ...
        barplot(-med, horiz = TRUE, width = 1, space = 0, yaxs = "i", 
            col = "gray30", xlim = c(-max(abs(mi), abs(mx)),
            max(abs(mi), max(mx))), ylim = c(0, dim(relative.weights)[1]),
            xlab = "", ylab = "", yaxt = "n", add = TRUE)

        # error bars
        arrows(y0 = c(seq_along(med)) - 0.5, x0 = -upp.qt, x1 = -low.qt, 
            angle = 90, code = 3, length = 0.04)
        mtext("median relative feat. weight", side = 1, line = 2, at = 0, 
            cex = 0.7, adj = 0.5)
        ...
        for (f in seq_along(sel.idx)) {
            t = paste(format(100 * sum(rel.weights[sel.idx[f], ] != 0)/
                dim(rel.weights)[2], digits = 1, scientific = FALSE),
                "%", sep = "")
        ...
        mtext("effect size", side = 3, line = 1, at = (mx/2), cex = 0.7, 
            adj = 1)
        ...
        barplot(-med, horiz = TRUE, width = 1, space = 0, yaxs = "i", 
            col = "gray30", xlim = c(0, mx), ylim = c(0,
...
#' @keywords internal
model.interpretation.pred.and.meta.plot <- function(prediction, label,
    meta = NULL, verbose = 0) {
    if (verbose > 2)
           ...
           cur.processed.data <- NULL
            if (all(is.numeric(img.data[, m]))) {
              ...
              if (length(levels(img.data[, m])) < 10){
                ...
                message("Metadata ",colnames(img.data)[m]," contains too many 
...
#' @keywords internal
model.interpretation.proportion.of.weights.plot <- function(selected.weights,
    ...
    if (verbose > 2)
        message("+ finished plotting model.interpretation.proportion.of.weights.plot")
}

# function to plot the percentage of features
#' @keywords internal
plot.percentage.of.features.plot <- function(selected.weights, all.weights, 
    verbose = 0) {
    if (verbose > 2)
...
#' @keywords internal
model.interpretation.heatmap.plot<- function(image.data, limits, color.scheme,
    effect.size,verbose = 0) {
    if (verbose > 2)
    ...
    if (!is.null(effect.size)) {
        for (f in seq_len(ncol(image.data))) {
            mtext(colnames(image.data)[f], side = 4, line = 1, 
                ...
                color.scheme[1 + 4], color.scheme[length(color.scheme) -4]))
        }
    } else {
        for (f in seq_len(ncol(image.data))) {
            mtext(colnames(image.data)[f], side = 4, line = 1, at = (f - 1)/
                (dim(image.data)[2] - 1), las = 2, cex = max(0.3,
...
#' @keywords internal
model.interpretation.prepare.heatmap.fc <- function(heatmap.data, limits, sel.feat,
    meta = NULL, label, detect.lim, verbose = 0) {
    if (verbose > 2)
        message("+ model.interpretation.prepare.heatmap.fc")
    if (!any(grepl("META", sel.feat))) {
        feat.log <- log10(heatmap.data[sel.feat,] + detect.lim)
        img.data <- t(feat.log - 
            log10(rowMedians(heatmap.data[sel.feat,label$n.idx]) + detect.lim))
    } else {
        img.data <- matrix(NA, nrow = length(sel.feat), 
        ...
        for (f in sel.feat) {
            if (verbose > 2)
                message(paste("+++", f))
            if (!grepl("META", f)) {
                median.ctr <- suppressWarnings(median(as.numeric(
                    heatmap.data[f, label$n.idx])))
                img.data[f, ] <- log10(heatmap.data[f, ] + detect.lim) - 
                log10(median.ctr + detect.lim)
            } else {
                meta.data <- meta[, grep(strsplit(f, "_")[[1]][2], 
                    colnames(meta), ignore.case = TRUE, value = TRUE)]
                # transform metadata to zscores
                meta.data <- apply(meta.data, c(1, 2), as.numeric)
                meta.data <- (meta.data - mean(meta.data, na.rm = TRUE))/
...
#' @keywords internal
model.interpretation.prepare.heatmap.zscore <- function(heatmap.data, limits, 
    verbose = 0) {
    if (verbose > 2)
...
#' @keywords internal
model.interpretation.select.features <- function(weights, model.type, 
    consens.thres, norm.models, label, max.show, verbose = 0) {
    ...
    if (model.type != "RandomForest") {
        ...
        median.sorted.features <- sort(med.weights[sel.idx], decreasing = TRUE,
            index.return = TRUE)
        ...
        weights <- t(t(weights)/colSums(abs(weights)))
        median.sorted.features <- sort(rowMedians(weights), decreasing = FALSE,
        index.return = TRUE)
        # take the feature with median higher than consens.threshold
        sel.idx <- median.sorted.features$ix[which(median.sorted.features$x >=
            consens.thres)]
    ...
    if (verbose > 2)
        message(paste("+++ generating plot for a model with", length(sel.idx),
         "selected features"))
    if (verbose > 2)
