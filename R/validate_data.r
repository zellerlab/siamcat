...
validate.data <- function(siamcat, verbose=1){
  ...
  if (length(siamcat@label@label) == dim(siamcat@phyloseq@otu_table)[2]){
    stopifnot(all(names(siamcat@label@label)
      %in% colnames(siamcat@phyloseq@otu_table))
      && all(colnames(siamcat@phyloseq@otu_table)
      %in% names(siamcat@label@label)))
    ...
    stopifnot(all(names(siamcat@label@label) ==
      colnames(siamcat@phyloseq@otu_table)))
    ...
    stopifnot(all(colnames(siamcat@phyloseq@otu_table)
      %in% names(siamcat@label@label)))

    ids <- colnames(siamcat@phyloseq@otu_table)[
      colnames(siamcat@phyloseq@otu_table) %in% names(siamcat@label@label)]
    # create new labels object with reduced labels
    siamcat <- filter.label(siamcat,ids)

  } else if (length(siamcat@label@label) <= dim(siamcat@phyloseq@otu_table)[2]){
    # if there are more samples in features, remove them
    #   and keep only the ones for which labels are available
    stopifnot(all(names(siamcat@label@label)
      %in% colnames(siamcat@phyloseq@otu_table)))

    cat('Warning: Removing',
      dim(siamcat@phyloseq@otu_table)[2]-length(siamcat@label@label),
  ...
  for (i in siamcat@label@info$class.descr){
    if(sum(siamcat@label@label==i) <= 5){
      stop("Data set has only",
        sum(siamcat@label@label==i),
        "training examples of class",i,
        " This is not enough for SIAMCAT to proceed")
      }
    if (sum(siamcat@label@label==i) < 10){
      cat("Data set has only",
        sum(siamcat@label@label==i),
        "training examples of class",i,
        " . Note that a dataset this small/skewed is not necessarily
  ...
  if (!is.null(siamcat@phyloseq@sam_data)) {
    ...
    if (length(siamcat@label@label) == dim(siamcat@phyloseq@sam_data)[1]){
      stopifnot(all(names(siamcat@label@label)
        %in% rownames(siamcat@phyloseq@sam_data))
        && all(rownames(siamcat@phyloseq@sam_data)
        %in% names(siamcat@label@label)))
      m <- match(names(siamcat@label@label),
        rownames(siamcat@phyloseq@sam_data))
      siamcat@phyloseq@sam_data <- siamcat@phyloseq@sam_data[m,]
      stopifnot(all(names(siamcat@label@label) ==
        rownames(siamcat@phyloseq@sam_data)))
    } else if (length(siamcat@label@label) <=
                dim(siamcat@phyloseq@sam_data)[1]) {
      stopifnot(all(names(siamcat@label@label)
        %in% rownames(siamcat@phyloseq@sam_data)))
      m <- match(names(siamcat@label@label),
        rownames(siamcat@phyloseq@sam_data))
      cat('Warning: Removing metadata information for',
        dim(siamcat@phyloseq@sam_data)[1]-length(siamcat@label@label),
        'superfluous samples.\n')
      siamcat@phyloseq@sam_data <- siamcat@phyloseq@sam_data[m,]
    } else {
