...
normalize.features <- function(siamcat,
    ...
    if (is.null(norm.param$norm.method)) {
        ...
        if (!norm.method %in% c("rank.unit", "rank.std", "log.std", "log.unit",
        "log.clr")) {
        ...
        if (norm.method == "rank.std" && is.null(norm.param$sd.min.q)) {
            stop("The rank.std method requires the parameter sd.min.q, which is
             not supplied. Exiting ...")
        }
        if (norm.method != "rank.std" && is.null(norm.param$log.n0)) {
            warning("Pseudo-count before log-transformation not supplied! 
        ...
        if (norm.method == "log.std" && (is.null(norm.param$sd.min.q))) {
            stop("The log.std method requires the parameters sd.min.q, which is
             not supplied. Exiting ...")
        }
        if (norm.method == "log.unit" && (is.null(norm.param$n.p) || 
            is.null(norm.param$norm.margin))) {
            stop("The log.std method requires the parameters n.p and 
        ...
        if (verbose > 1)
            message(paste0("+ feature sparsity before normalization: ", 
                formatC(100 * mean(feat.red == 0), digits=4), "%"))
        # normalization
        if (verbose > 2)
            message("+++ performing normalization")
        if (norm.method == "rank.unit") {
            feat.rank <- colRanks(feat.red, preserveShape = TRUE, 
                ties.method = 'average')
            ...
            if (norm.param$norm.margin == 1) {
                ...
                stop("Unknown margin for normalization, must be either 
                    1 (for features), 2 (for samples), or 3 (global). 
                    Exiting...")
            }
        }
        if (verbose > 1)
            message(paste("+++ feature sparsity after normalization: ", 
        ...
        if (verbose > 1)
            message(paste("+ performing frozen ", norm.param$norm.method, 
        ...
        if (norm.param$norm.method == "rank.unit") {
            ...
            stopifnot(!is.null(norm.param$log.n0) && 
                !is.null(norm.param$geometric.mean) && 
                all(names(norm.param$geometric.mean) ==
                row.names(feat.red)))
            ...
            stopifnot(!is.null(norm.param$feat.mean) && 
                !is.null(norm.param$feat.adj.sd) && 
                all(names(norm.param$feat.mean) ==
                row.names(feat.red)) && all(names(norm.param$feat.adj.s) == 
                row.names(feat.red)))
            feat.rank <- colRanks(feat.red, preserveShape = TRUE, 
                ties.method = 'average')
            ...
            stopifnot(!is.null(norm.param$log.n0) && 
                ...
                c(1, 2, 3) && ((norm.param$norm.margin == 1 && 
                    !is.null(norm.param$feat.norm.denom) && 
                    all(names(norm.param$feat.norm.denom) ==
                row.names(feat.red))) || (norm.param$norm.margin == 2 && 
                !is.null(norm.param$norm.fun)) || (norm.param$norm.margin ==
        ...
        if (verbose > 1)
            message(paste0("+ feature sparsity after normalization: ", 
                formatC(100 * mean(feat.norm == 0), digits=4), "%"))
        stopifnot(!any(is.na(feat.norm)))
    ...
    e.time <- proc.time()[3]
    if (verbose > 1)
        message(paste("+ finished normalize.features in", 
