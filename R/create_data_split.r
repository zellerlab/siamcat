...
create.data.split <- function(siamcat, num.folds = 2, num.resample = 1,
    ...
    for (r in seq_len(num.resample)) {
        ...
        foldid <- assign.fold(label = labelNum, num.folds = num.folds,
                              stratified = stratify, inseparable = inseparable,
...
#' @keywords internal
assign.fold <- function(label, num.folds, stratified, inseparable = NULL,
                        meta = NULL, verbose = 1) {
    if (verbose > 2)
