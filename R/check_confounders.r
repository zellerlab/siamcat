...
check.confounders <- function(siamcat, fn.plot, verbose = 1) {
    ...
    case.count <- sum(label$label == p.label)
    ctrl.count <- sum(label$label == n.label)

    if (verbose > 2)
        message("+++ checking group sizes")
    if (case.count > ctrl.count) {
        lgr <- which(label$label == p.label)
        smlr <- which(label$label == n.label)
        bp.labs <- rev(names(label$info))
    } else {
        lgr <- which(label$label == n.label)
        smlr <- which(label$label == p.label)
        bp.labs <- names(label$info)
    }
    ...
    for (m in seq_len(ncol(meta(siamcat)))) {
        ...
        if (length(u.val) == 1) {
            if (verbose > 1) {
                ...
                legend(2.5, 1, legend = u.val, xpd = NA, lwd = 2,
                    col = colors, inset = 0.5, bg = "grey96", cex = 0.8)
                ...
                vps <- baseViewports()
                pushViewport(vps$figure)
                niceLabel <- rep(names(label$info)[2], length(label$label))
                names(niceLabel) <- names(label$label)
                niceLabel[label$label == n.label] <- names(label$info)[1]
                vp1 <- plotViewport()
                t <-
                    addmargins(table(mvar, niceLabel, dnn = c(mname, "Label")))
            ...
            qqplot(mvar[label$label == n.label], mvar[label$label == p.label],
                xlim = ax.int, ylim = ax.int, pch = 16, cex = 0.6,
                xlab = names(label$info)[1], ylab = names(label$info)[2],
                main = paste("Q-Q plot for", mname))
            abline(0, 1, lty = 3)
            p.val <-
                wilcox.test(mvar[label$label == n.label],
            ...
            hist(mvar[label$label == n.label], main = names(label$info)[1],
                xlab = mname, col = histcolors,
                breaks = seq(min(mvar, na.rm = TRUE),
                    max(mvar, na.rm = TRUE), length.out = 10))
            mtext(paste("N =", length(mvar[label$n.idx])), cex = 0.6,
                side = 3, adj = 1, line = 1)
            ...
            combine <-
                data.frame(mvar[lgr], c(mvar[smlr], rep(NA, len.diff)))


            boxplot(combine[, 1], na.omit(combine[, 2]), range = 1.5,
                use.cols = TRUE, names = bp.labs, ylab = mname,
                main = paste("Boxplot for", mname), col = histcolors,
                outpch = NA)
            ...
            hist(mvar[label$label == p.label], main = names(label$info)[2],
                xlab = mname, col = histcolors,
                breaks = seq(min(mvar, na.rm = TRUE), max(mvar, na.rm = TRUE),
                    length.out = 10))
            mtext(paste("N =", length(mvar[label$label == p.label])),
    ...
    if (verbose == 1) {
        message(
            paste(
                "Checking metadata for confounders finished, results
                plotted to:",
                fn.plot
            )
        )
