...
#'     plot, depending on the type of metadata. The conditional entropy 
...
#' @usage check.confounders(siamcat, fn.plot, meta.in = NULL, max.num = 10, 
#'     verbose = 1) 
#' @param siamcat an object of class \link{siamcat-class}
#' @param fn.plot string, filename for the pdf-plot
#' @param meta.in vector, specific metadata variable names to analyze, 
#'     defaults to NULL 
#' @param max.num integer, max number of metadata variables to analyze, 
#'     defaults to 10
...
#'     check.confounders(siamcat_example, './conf_plot.pdf', 
#'     verbose=2)}
check.confounders <- function(siamcat, fn.plot, meta.in = NULL, max.num = 10,
                              verbose = 1) {
    ...
    s.time <- proc.time()[3]
    label <- label(siamcat)
    meta <- factorize.metadata(meta(siamcat)) # creates data.frame
    
    # check validity of input metadata conditions
    if (is.null(meta.in) && (max.num <= 10)) {
        message(paste0("+ preparing to analyze top ", length(colnames(meta)), 
                       " metadata associations"))}
    else if (is.null(meta.in) && (max.num > 10)) {
        warning("Maximum number of metadata variables for analysis is 10. 
                Continuing with cutoff set at 10.")
        max.num <- 10}
    else if (!is.null(meta.in) && 
             length(meta.in[which(meta.in %in% colnames(meta))]) <= 10) {
        if (!all(meta.in %in% colnames(meta))) {
            meta.in <- meta.in[which(meta.in %in% colnames(meta))]
            warning("Some specified metadata were not in metadata file.
                    Continuing with: ", paste(c(meta.in), collapse=" "))}
        else {
            message("+ preparing to analyze specified metadata associations")}
        max.num <- length(meta.in)
        meta <- meta[,meta.in]}
    else if (!is.null(meta.in) && 
             length(meta.in[which(meta.in %in% colnames(meta))]) > 10) {
        warning("Maximum number of metadata variables for analysis is 10. 
                Continuing with top 10 from specified list.")
        meta.in <- meta.in[which(meta.in %in% colnames(meta))]
        max.num <- 10
        meta <- meta[,meta.in]}
    ...
    if ((verbose > 1) & (length(names(which(indep == 0))) > 0)){
        message("++ label object derived from ", 
                paste(c(names(which(indep == 0))), collapse = " & "), 
                " variables which have been removed from further analysis")}
    meta <- meta[,names(which(indep != 0))]
    meta(siamcat) <- sample_data(as.data.frame(meta(siamcat)[,colnames(meta)]))
...
confounders.corrplot <- function(meta, label) {
    ...
    for (c in seq_along(colnames(meta.temp))) {
        entropies[,c] <- vapply(colnames(meta.temp), FUN=function(x) {
            idx <- intersect(which(!is.na(meta.temp[,c]) == TRUE), 
                             which(!is.na(meta.temp[,x]) == TRUE))
            return(condentropy(discretize(meta.temp[,x][idx]), 
                               discretize(meta.temp[idx,c])))},
            FUN.VALUE = numeric(1))}
    ...
    col = colorRampPalette(brewer.pal(brewer.pal.info['Blues', 
                                                      'maxcolors'], 'Blues'))(100)
    corrplot(entropies,
...
confounders.build.glms <- function(meta, label) {
    ...
    return(list("reg.ci" = reg.ci[which(lapply(reg.ci, is.null) == FALSE)],
                ...
                "colors" = brewer.pal(length(colnames(meta)), "Spectral"),
...
confounders.glm.reg.coef.plot <- function(glm.data) {
    ...
    x.ticks <- seq(margins[1], margins[2], length.out = 10)
    x.tick.labels <- formatC(x.ticks, digits=2)
    #x.tick.labels <- parse(text = paste0('e^', signif(x.ticks, digits = 2)))
    y.ticks <- seq(1, length(order))
    
    text.names <- vapply(order, FUN=function(x) {
        x <- tolower(x)
        return(gsub('[_.-]', ' ', paste(toupper(substring(x, 1, 1)),
                                        substring(x, 2), sep = "")))}, FUN.VALUE = character(1))
    ...
    for (i in seq_along(order)) {
        if (!is.na(glm.data$reg.coef[order[i]])) {
            ...
            points(glm.data$reg.coef[order[i]], i, pch = 18, col=glm.data$colors[i])
...
confounders.glm.reg.pval.plot <- function(glm.data) {
    ...
    x.ticks <- seq(0, x.max, length.out = 10)
    x.tick.labels <- parse(text = paste0('10^', signif(x.ticks*-1, digits = 2)))
    ...
    for (m in seq_along(meta)) {
        ...
        if (length(u.val) == 1) {
            ...
            text(ax.int[1] + 0.9 * (ax.int[2] - ax.int[1]),
                 ...
                 cex = 0.8,paste("MWW Test P Value:", format(p.val, digits = 4)),
                 pos = 2)
