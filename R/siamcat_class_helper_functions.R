...
filter.label <- function(siamcat, ids, verbose = 1) {
  ...
  labels_new <- list(label = label_old$label[ids], 
                    header = label_old$header, info = label_old$info,
...
setMethod("show", "siamcat", function(object) {
  ...
  if (!is.null(label(object)))
    cat(paste("label()                label:           ", 
      sum(label(object)$n.idx), label(object)$n.lab, "and",
      sum(label(object)$p.idx),
              label(object)$p.lab, "samples", sep = " "), fill = TRUE)
  ...
  if (length(eval_data(object))) {
    cat(paste("eval_data()             eval_data:         Average AUC:", 
      round(eval_data(object)$auc.average[[1]], 3), sep = " "), fill = TRUE)
  }
  ...
  if (!is.null(sample_data(physeq(object), FALSE))) {
    cat(paste("phyloseq@sam_data()    Sample Data:       [ ", 
      dim(sample_data(physeq(object)))[1], " samples by ",
              dim(sample_data(physeq(object)))[2], " sample variables ]",
               sep = ""), fill = TRUE)
  }
  ...
  if (!is.null(tax_table(physeq(object), FALSE))) {
    cat(paste("phyloseq@tax_table()   Taxonomy Table:    [ ",
      dim(tax_table(physeq(object)))[1], " taxa by ",
              dim(tax_table(physeq(object)))[2], " taxonomic ranks ]", 
              sep = ""), fill = TRUE)
  }
