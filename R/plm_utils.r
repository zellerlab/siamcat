...
#' @keywords internal
train.plm <- function(data, method = c("lasso", "enet", "ridge", "lasso_ll",
 "ridge_ll", "randomForest"), measure = list("acc"),
    ...
    cl <- "classif.cvglmnet"  
    parameters <- get.parameters.from.param.set(param.set = param.set, 
    ...
    if (method == "lasso") {
        ...
        lrn <- makeLearner(cl, predict.type = "prob", epsilon = 1e-08, 
            wi = class.weights)
        ...
        stop(method, " is not a valid method, currently supported: lasso, 
    ...
    if (!all(is.null(parameters))) {
        hyperPars <- tuneParams(learner = lrn, task = task, 
    ...
    if (cl == "classif.cvglmnet") {
        ...
        model$feat.weights <- model$learner.model$W[
...
#' @keywords internal
get.optimal.lambda.for.glmnet <- function(trained.model, training.task, 
    perf.measure, min.nonzero.coeff) {
    # get lambdas that fullfill the minimum nonzero coefficients criterion
    lambdas <- trained.model$learner.model$glmnet.fit$lambda[
    which(trained.model$learner.model$nzero >= min.nonzero.coeff)]
    # get performance on training set for all those lambdas in trace
    performances <- vapply(lambdas, FUN = function(lambda, model, task, 
        measure) {
        ...
        if (is.null(model.transformed$learner$par.vals$s)) {
            ...
            model.transformed$learner$par.vals$s]] <- lambda
        }
     ...
     USE.NAMES=FALSE, FUN.VALUE=double(1))
...
get.parameters.from.param.set <- function(param.set, method, sqrt.mdim) {
    ...
    if (method == "lasso_ll") {
        ...
        parameters <- makeParamSet(makeNumericParam("ntree", lower = ntree[1],
         upper = ntree[2]), makeDiscreteParam("mtry",
            values = mtry))
    } else if (method == "enet") {
        parameters <- makeParamSet(makeNumericParam("alpha", lower = alpha[1],
         upper = alpha[2]))
    }
    return(parameters)
}
