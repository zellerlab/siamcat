...
evaluate.predictions <- function(siamcat, verbose = 1) {
    ...
    s.time <- proc.time()[3]
    # TODO compare header to label make sure that label and prediction are in the same order stopifnot(all(names(label)
    # %in% rownames(pred)) && all(rownames(pred) %in% names(label)))
