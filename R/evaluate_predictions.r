...
evaluate.predictions <- function(siamcat, verbose = 1) {
    ...
    if (ncol(pred) > 1) {
        ...
        for (c in seq_len(ncol(pred))) {
            rocc[c] = list(roc(response = label$label, predictor = pred[, c],
             ci = FALSE))
            aucs[c] = rocc[[c]]$auc
        }
    ...
    summ.stat = "mean"
    rocsumm = list(roc(response = label$label, 
        predictor = apply(pred, 1, summ.stat), ci = TRUE, of = "se",
        sp = seq(0, 1, 0.05)))
    ...
    if (ncol(pred) > 1) {
        ...
        ev = append(ev, list(evaluate.classifier(apply(pred, 1, summ.stat),
         label$label, label)))
    } else {
    ...
    e.time <- proc.time()[3]
    if (verbose > 1)
        message(paste("+ finished evaluate.predictions in", 
...
evaluate.classifier <- function(predictions, test.label, label, verbose = 0) {
    ...
    if (is.null(dim(predictions))) {
        ...
        tp = vapply(thr, FUN=function(x){sum(test.label == label$positive.lab
         & predictions > x)}, USE.NAMES = FALSE, FUN.VALUE = integer(1))
        fp = vapply(thr, FUN=function(x){sum(test.label == label$negative.lab
         & predictions > x)}, USE.NAMES = FALSE, FUN.VALUE = integer(1))
        tn = vapply(thr, FUN=function(x){sum(test.label == label$negative.lab
         & predictions < x)}, USE.NAMES = FALSE, FUN.VALUE = integer(1))
        fn = vapply(thr, FUN=function(x){sum(test.label == label$positive.lab
         & predictions < x)}, USE.NAMES = FALSE, FUN.VALUE = integer(1))
    } else {
        ...
        tp = t(tp = vapply(thr, FUN=function(x){apply(predictions, 2, 
            FUN=function(y){sum(test.label == label$positive.lab & y > x)})},
            USE.NAMES = FALSE, FUN.VALUE = integer(2)))
        fp = t(tp = vapply(thr, FUN=function(x){apply(predictions, 2,
            FUN=function(y){sum(test.label == label$negative.lab & y > x)})},
            USE.NAMES = FALSE, FUN.VALUE = integer(2)))
        tn = t(tp = vapply(thr, FUN=function(x){apply(predictions, 2,
            FUN=function(y){sum(test.label == label$negative.lab & y < x)})},
            USE.NAMES = FALSE, FUN.VALUE = integer(2)))
        fn = t(tp = vapply(thr, FUN=function(x){apply(predictions, 2,
            FUN=function(y){sum(test.label == label$positive.lab & y < x)})},
            USE.NAMES = FALSE, FUN.VALUE = integer(2)))
    }
    if (verbose > 2)
