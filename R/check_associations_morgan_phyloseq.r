...
check.associations <- function(siamcat, fn.plot, color.scheme="RdYlBu",
  ...
  if (is.null(siamcat@phyloseq@sam_data)){
    result.list <- marker.analysis.features(siamcat,
    ...
    result.list <- marker.analysis.with.metadata(siamcat,
                                                 detect.lim=detect.lim,
                                                 pr.cutoff=pr.cutoff,
                                                 mult.corr=mult.corr,
                                                 alpha=alpha,
                                                 ...
                                                 max.show=max.show,
                                                 col=col)
    aucs.meta <- result.list$aucs.meta
  }
...
marker.analysis.features <- function(siamcat,
                                     detect.lim, colors,
...
marker.analysis.with.metadata <- function(siamcat,
                                          feat, label, detect.lim, colors,
                                          pr.cutoff, mult.corr, alpha,
                                          max.show, sort.by) {
  
  meta.colnames <- unique(siamcat@phyloseq@sam_data@names)
  siamcat.meta.mat <- as.matrix(siamcat@phyloseq@sam_data, nrow=nrow(siamcat@phyloseq@sam_data, ncol=ncol(siamcat@phyloseq@sam_data)))
  ...
  p.val.list  <- vector('list', nrow(feat))
  ...
  for (f in 1:nrow(feat)) {
    ...
    for (c in meta.colnames) {
      meta.cat <- siamcat.meta.mat[,c]
      continuous <- FALSE

      # split continuous metadata
      if (length(unique(meta.cat[!is.na(meta.cat)])) > 5) {
        continuous <- TRUE
        quartiles <- quantile(meta.cat, probs=seq(0,1,0.25), na.rm=TRUE)
        meta.cat.disc <- cut(meta.cat, breaks=quartiles, labels=1:4, include.lowest=TRUE)
      }
      
      # set up blocking variable
      if (continuous==TRUE) {
        block <- as.factor(meta.cat.disc)
      } else {
        block <- as.factor(meta.cat)
      }
      ...
      for (n in 1:length(block)) {
        if (is.na(block[n])) {
          na.samples <- c(na.samples, names(meta.cat[n]))
        }
      }
      block <- block[!names(siamcat@label@label) %in% na.samples]
      
      # block for this metadata category and add that p-value to matrix
      d <- data.frame(y=feat[f,which(!names(siamcat@label@label) %in% na.samples)], 
                      x=as.factor(siamcat@label@label[!(names(siamcat@label@label) %in% na.samples)]), 
                      block)
      p.val.mat[f,c] <- pvalue(wilcox_test(y ~ x | block, data=d))
      

      # for (s in 1:length(meta.subcat[c])){
      # 
      #   # named int of sample IDs & indeces in which e.g. gender==2
      #   m.names <- which(siamcat.meta.mat[,c]==s)
      #   print(s)
      # 
      #   x <- feat[f, (names(siamcat@label@p.idx) %in% names(m.names))] #& siamcat@label@p.idx)]
      #   y <- feat[f, (names(siamcat@label@n.idx) %in% names(m.names))] #& siamcat@label@n.idx)]
      # 
      #   # Wilcoxon
      #   p.val.list[[f]][[s]] <- wilcox.test(x, y, exact=FALSE)$p.value
      # 
      #   # AUC
      #   aucs[[f]][[s]] <- c(roc(controls=y, cases=x, direction='<', ci=TRUE, auc=TRUE)$ci)
      # 
      #   # Prevalence-shift
      #   temp.n <- sum(y >= pr.cutoff)/length(y)
      #   temp.p <- sum(x >= pr.cutoff)/length(x)
      #   pr.shift[f,s] <- temp.p-temp.n
      # 
      #   # FC
      #   q.p <- quantile(log10(x+detect.lim), probs=seq(.1, .9, .05))
      #   q.n <- quantile(log10(y+detect.lim), probs=seq(.1, .9, .05))
      #   fc[f, s] <- sum(q.p - q.n)/length(q.p)
      # }
            
    }
    
    p.val.mat[f,"no block"] <- wilcox.test(feat[f,siamcat@label@n.idx], feat[f,siamcat@label@p.idx], exact = FALSE)$p.value
  }
}
