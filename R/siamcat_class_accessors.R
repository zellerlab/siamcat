...
accessSlot <- function(siamcat, slot, verbose=1) {
    ...
    if (slot %in% c(
        'eval_data', 'data_split', 'model_list', 'norm_feat', 'filt_feat', 'ordination')){
        if (length(out) == 0) {
            out <- NULL
        }
    }
...
###############################################################################
#' @title Retrieve ordination
#'
#' @description Function to retrieve the SIAMCAT ordination for the
#' contained phyloseq object as generated by make.ordination.
#'
#' @usage ordination(siamcat)
#'
#' @param siamcat (Required). A \link{siamcat-class} object
#' that contains evaluation data
#'
#' @return A list with ordination ("ord") and parameters ("method", "distance").
#'
#' @export
#'
#' @rdname ordination-methods
#'
#' @docType methods
#'
#' @examples
#' ordination(siamcat_example)
setGeneric("ordination", function(siamcat, verbose=1)
    standardGeneric("ordination"))
setMethod("ordination", "siamcat", function(siamcat, verbose=1) {
    accessSlot(siamcat, "ordination", verbose)
})
